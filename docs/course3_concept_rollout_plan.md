# Math Course 3 Concept Rollout Plan

Last updated: 2026-07-31

## Purpose

This plan defines the complete Course 3 mathematics pathway and the order in
which learner-ready concept packs should be built.

Course 3 is intended for a Grade 7 student working on an accelerated Grade 8
instructional pathway. This preserves the platform distinction between a
student's `homeGrade` and `instructionalGrade`.

The plan follows the existing Course 2 release model:

1. tutorial exists
2. core test exists
3. review test exists
4. scoring and answer-key validation pass
5. concept becomes learner-ready

Only learner-ready concepts belong in the active Course 3 manifest. The full
future sequence remains in this plan until each concept pack is complete.

## Curriculum Basis

The sequence is aligned to the Grade 8 Common Core State Standards for
Mathematics and the California Mathematics Framework.

Primary sources:

- [Common Core State Standards for Mathematics](https://corestandards.org/wp-content/uploads/2023/09/Math_Standards1.pdf)
- [California Mathematics Framework](https://www.cde.ca.gov/ci/ma/cf/)
- [California Grade Eight Mathematics Framework chapter](https://www.cde.ca.gov/ci/ma/cf/documents/mathfwgrade8jl.pdf)

Grade 8 mathematics emphasizes three connected areas:

- expressions, linear equations, systems, and bivariate linear models
- functions as relationships between quantities
- geometry involving transformations, similarity, the Pythagorean Theorem,
  and three-dimensional volume

The standards also require work with irrational numbers, radicals, integer
exponents, scientific notation, and bivariate data.

## Course Metadata

- Subject: Mathematics
- Course: Course 3
- Learner placement: accelerated Grade 7
- Instructional level: Grade 8
- Standards framework: CA-CCSSM / Common Core Grade 8
- Delivery model: local-first, concept-based, deterministic practice

## Complete Course Sequence

### Unit 1 — Real Numbers and Exponents

Standards: `8.NS.1–2`, `8.EE.1–4`

1. Rational and Irrational Numbers
2. Repeating Decimals as Fractions
3. Square Roots and Perfect Squares
4. Cube Roots and Perfect Cubes
5. Approximate Irrational Numbers
6. Compare and Order Real Numbers
7. Integer Exponents
8. Zero and Negative Exponents
9. Powers of Ten
10. Scientific Notation
11. Operations with Scientific Notation
12. Unit 1 Mixed Review

### Unit 2 — Linear Equations in One Variable

Standards: `8.EE.7`

1. Review Equivalent Expressions
2. Solve Multi-Step Equations
3. Equations with the Distributive Property
4. Equations with Variables on Both Sides
5. Equations with Rational Coefficients
6. One Solution, No Solution, or Infinitely Many Solutions
7. Build Equations from Context
8. Linear Equation Word Problems
9. Check and Explain Equation Solutions
10. Unit 2 Mixed Review

### Unit 3 — Lines, Slope, and Linear Equations

Standards: `8.EE.5–6`

1. Proportional Relationships as Lines
2. Understand Slope as Rate of Change
3. Find Slope from Graphs
4. Find Slope from Tables and Points
5. Compare Rates of Change
6. Similar Triangles and Constant Slope
7. Equations in the Form `y = mx`
8. Understand the `y`-Intercept
9. Equations in the Form `y = mx + b`
10. Graph Linear Equations
11. Write Linear Equations from Representations
12. Unit 3 Mixed Review

### Unit 4 — Systems of Linear Equations

Standards: `8.EE.8`

1. Understand Solutions to Systems
2. Solve Systems by Graphing
3. Solve Systems by Substitution
4. Solve Systems by Elimination
5. Systems with No Solution
6. Systems with Infinitely Many Solutions
7. Choose an Efficient System-Solving Method
8. Systems of Equations Word Problems
9. Estimate and Check System Solutions
10. Unit 4 Mixed Review

### Unit 5 — Functions

Standards: `8.F.1–5`

1. Understand Functions
2. Functions in Tables and Mapping Diagrams
3. Functions on Graphs
4. Compare Functions in Different Representations
5. Linear and Nonlinear Functions
6. Construct Linear Functions
7. Rate of Change and Initial Value
8. Interpret Linear Functions in Context
9. Describe Increasing and Decreasing Relationships
10. Sketch Graphs from Verbal Descriptions
11. Function Modeling Word Problems
12. Unit 5 Mixed Review

### Unit 6 — Transformations, Congruence, and Similarity

Standards: `8.G.1–5`

1. Translations on the Coordinate Plane
2. Reflections on the Coordinate Plane
3. Rotations on the Coordinate Plane
4. Sequences of Rigid Transformations
5. Congruent Figures
6. Dilations on the Coordinate Plane
7. Scale Factor and Dilations
8. Similar Figures
9. Sequences of Transformations and Similarity
10. Parallel Lines and Transversals
11. Triangle Angle Relationships
12. Exterior Angles of Triangles
13. Angle-Angle Similarity
14. Unit 6 Mixed Review

### Unit 7 — Pythagorean Theorem and Volume

Standards: `8.G.6–9`

1. Explore the Pythagorean Theorem
2. Explain the Pythagorean Theorem
3. Find Missing Legs and Hypotenuses
4. Converse of the Pythagorean Theorem
5. Pythagorean Theorem Word Problems
6. Distance on the Coordinate Plane
7. Three-Dimensional Pythagorean Applications
8. Volume of Cylinders
9. Volume of Cones
10. Volume of Spheres
11. Composite Volume Problems
12. Unit 7 Mixed Review

### Unit 8 — Bivariate Data

Standards: `8.SP.1–4`

1. Read and Construct Scatter Plots
2. Patterns of Association
3. Clusters and Outliers
4. Informal Lines of Best Fit
5. Evaluate the Fit of a Linear Model
6. Use Linear Models to Make Predictions
7. Interpret Slope and Intercept in Data
8. Two-Way Tables
9. Relative Frequencies in Two-Way Tables
10. Categorical Association
11. Bivariate Data Word Problems
12. Unit 8 Mixed Review
13. Course 3 Final Review

## Phased Build Order

### Phase 1 — Real Number Foundation

Build Unit 1 in sequence. These concepts establish the number sense needed for
radicals, scientific notation, equations, geometry, and later Algebra I work.

The current learner-ready packs are:

- Rational and Irrational Numbers
- Repeating Decimals as Fractions
- Square Roots and Perfect Squares
- Cube Roots and Perfect Cubes
- Approximate Irrational Numbers
- Compare and Order Real Numbers
- Integer Exponents
- Zero and Negative Exponents
- Powers of Ten
- Scientific Notation
- Operations with Scientific Notation
- Unit 1 Mixed Review

Unit 1 is complete.

### Phase 2 — Linear Equation Foundation

Build Unit 2 before formal slope and systems work. Preserve explicit reasoning
about equivalent equations instead of teaching only mechanical steps.

The current learner-ready Unit 2 packs are:

- Review Equivalent Expressions
- Solve Multi-Step Equations
- Equations with the Distributive Property
- Equations with Variables on Both Sides
- Equations with Rational Coefficients
- One Solution, No Solution, or Infinitely Many Solutions
- Build Equations from Context
- Linear Equation Word Problems
- Check and Explain Equation Solutions
- Unit 2 Mixed Review

Unit 2 is complete.

The current learner-ready Unit 3 packs are:

- Proportional Relationships as Lines
- Understand Slope as Rate of Change
- Find Slope from Graphs
- Find Slope from Tables and Points
- Compare Rates of Change
- Similar Triangles and Constant Slope
- Equations in the Form `y = mx`
- Understand the `y`-Intercept
- Equations in the Form `y = mx + b`
- Graph Linear Equations
- Write Linear Equations from Representations
- Unit 3 Mixed Review

Unit 3 is complete.

The current learner-ready Unit 4 packs are:

- Understand Solutions to Systems
- Solve Systems by Graphing
- Solve Systems by Substitution
- Solve Systems by Elimination
- Systems with No Solution

The next Course 3 rollout work is Unit 4:

- Systems with Infinitely Many Solutions

Course 3 multiple-choice answer positions should remain balanced within each
active test set so learners cannot infer the correct option from repeated
placement. Current Course 3 core banks target a 13/13/12/12 split across
A/B/C/D, and review banks target 5/5/5/5. Correct-answer positions must also
avoid predictable local sequences such as A/B/C/D/A/B/C/D, shorter A/B/C or
D/A/B mini-cycles, and long repeated answer-position runs.

Course 3 test banks must vary question forms enough that practice does not feel
like one template with changed numbers. The automated content quality gate now
checks normalized question-template variety, maximum single-template share, and
challenge-question reasoning variety. Some already-published banks are tracked
as explicit template-variety debt in the test suite; do not add new banks to
that list as a substitute for authoring varied questions.

Course 3 tutorials must also be concept-specific. The automated content quality
gate checks active Course 3 tutorial length and pairwise similarity so copied
or near-copied tutorial bodies cannot be released by changing only a heading.

The twelve learner-ready Unit 3 packs have been regenerated or authored
under this gate and should remain off the template-variety debt list.

### Phase 3 — Lines, Slope, and Systems

Build Units 3 and 4 as a connected sequence:

- proportional relationships become linear equations
- slope becomes constant rate of change
- pairs of linear equations become systems

### Phase 4 — Functions

Build Unit 5 after students can interpret slope and intercept. Use tables,
graphs, equations, and verbal descriptions throughout.

### Phase 5 — Geometry

Build Units 6 and 7. Coordinate transformations should come before congruence
and similarity; square-root fluency should come before Pythagorean applications.

### Phase 6 — Bivariate Data and Cumulative Review

Build Unit 8 last because its linear models rely on rate-of-change and
intercept understanding developed earlier.

## Definition of Ready

Before a Course 3 concept enters the manifest:

- tutorial is complete and readable by a Grade 7 learner
- core test contains challenging, balanced practice
- review test reinforces the essential ideas with lower cognitive load
- all question IDs are globally unique
- every multiple-choice correct answer exactly matches one authored option
- distractors represent realistic misconceptions
- explanations state why the correct answer works
- difficulty distribution is intentional
- answer-position sequence checks pass, not just total answer-position balance
- question-template variety checks pass unless reducing documented legacy debt
- known-correct answer paths are spot-checked
- repository-wide content validation passes

## Content Quality Expectations

Each concept pack should:

- teach meaning before procedure
- connect representations rather than isolate them
- include real-world reasoning without artificial story problems
- include misconception analysis and error correction
- avoid relying on one repeated question template
- use Grade 7-friendly language while preserving Grade 8 rigor
- remain deterministic and reviewable

## UI and Navigation Direction

Course navigation should group courses under their subject so Mathematics can
contain Course 2 and Course 3 without appearing as two separate subjects.

The learner should be able to:

1. choose Mathematics
2. choose Course 2 or Course 3
3. see units and learner-ready concepts
4. open a tutorial or test without encountering incomplete content

Progress should remain student-scoped and course-aware.
